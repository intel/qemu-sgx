                   ...
                   uint32_t *ecx, uint32_t *edx)
{
    ...
    case 0x12:
         ...
         * dependent on hardware and system (KVM) support.  For most entries
         * we rely entirely on KVM to manage the CPUID values as there is
         * (currently) no value in deviating from hardware, and if there
         * were some perceived value, e.g. restricting SECS.ATTRIBUTES.XFRM,
         * it would require KVM support that does not exist, e.g. trapping
         * ECREATE to enforce the user-specified restrictions.  The exception
         * is the SGX features word (see below).
         */
